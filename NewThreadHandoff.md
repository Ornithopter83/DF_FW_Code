...
- TM 소스는 V045로 관리한다. 별도 `충전 기준 보정` 버튼으로 3초 정지 확인과 2초 기준값 수집을 수행하며, Z축 자이로를 감쇠되는 가상 Roll로 변환해 보정 Roll과 합친 게임 좌우값을 표시한다. 이번 TM 단계에서는 작업자가 충전 상태를 확인하고 버튼으로 시작한다. Visual Studio `FolderProfile`로 V045 단일 EXE를 직접 게시할 수 있다. V045 빌드·게시·self-test·장비 검증은 미실행이며, 마지막 생성 실행 파일은 `DFTestModule_V040.exe`다.
