...
## 작업 11-W 모터 화면 왼쪽 정렬·BLDC 행 분리 완료 (2026-09-04)

- 모터 화면의 강제 들여쓰기와 열 맞춤용 빈 공간을 제거하고 설정·버튼 행을 모두 왼쪽 정렬로 변경했다.
- BLDC는 라벨을 복원해 `방향`, `Duty`, `시간(ms)`을 각각 별도 행으로 나눴다.
- BLDC 출력·정지와 동시 출력은 분리된 설정 행 아래에서 왼쪽 정렬을 유지한다.
- TM 버전은 `V041`을 유지했다. 빌드·게시·화면 검증은 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 11-V 모터 행·버튼 정렬 완료 (2026-09-04)

- 메인 모터와 BLDC의 출력·정지 버튼을 각각 설정 행 바로 아래 행으로 내렸다.
- BLDC의 CW/CCW 영역과 같은 폭의 공간을 메인 행 앞에 두어 `메인 모터 Duty`와 `BLDC Duty`의 시작 위치를 세로로 맞췄다.
- BLDC Duty 슬라이더를 메인 모터와 같은 210px 길이로 확대했다.
- `동시 출력`은 페이지 맨 아래가 아니라 BLDC 출력·정지 바로 아래 행에 같은 시작 위치로 배치했다.
- TM 버전은 `V041`을 유지했다. 빌드·게시·화면 검증은 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 11-U 모터 화면 재배치·슬라이더 테두리 완료 (2026-09-04)

- 메인 모터의 동작시간을 제거하고 Duty 숫자·테두리 슬라이더·출력·정지 버튼을 같은 줄로 올렸다. 출력은 시간 0의 연속 명령을 사용한다.
- BLDC 방향 라벨을 제거하고 CW/CCW부터 Duty·테두리 슬라이더·시간·출력·정지까지 한 줄에 배치했다.
- `동시 출력` 버튼을 복원해 현재 메인 Duty와 BLDC 방향·Duty·시간을 순서대로 송신한다.
- 각 정지 버튼은 화면 Duty를 0으로 바꾸고 기존 0 출력 명령을 송신한다.
- TM 버전은 사용자 지정 `V041`을 유지한다. 빌드·게시·self-test·화면 검증은 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 11-T 모터 독립 슬라이더 제어·TM V041 고정 완료 (2026-09-04)

- 모터 동시 제어 화면을 메인 모터와 BLDC 독립 제어 화면으로 분리했다. 두 Duty 숫자 입력에 0~255 슬라이더를 추가하고 양쪽 값을 연동했다.
- 메인 모터와 BLDC에 각각 별도 `출력`·`정지` 버튼을 연결했다.
- BLDC는 `방향 → BLDC Duty → 동작시간`을 한 줄에 배치하고 다음 줄에 출력·정지 버튼을 배치했다.
- 사용자 지정에 따라 TM 버전을 `V041`로 설정했으며, 사용자가 별도 버전을 지정하기 전까지 같은 값을 유지한다.
- 빌드·게시·self-test·화면 검증은 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 12-A Main IMU 게임 좌우 자동 보정 완료 (2026-09-04)

- Main에 충전 정지 상태의 자이로 바이어스 보정과 Z축 자이로 기반 가상 Roll을 구현했다. 게임 `$09`의 첫 Roll 값만 `현재 Roll + 가상 Roll`로 바꾸고 TM에는 원본 payload를 유지한다.
- 기존 배터리 상승 기반 충전 판정에서 자동 보정을 시작하며, 필요하면 보정 시간 동안 IMU를 임시로 켠다. 게임 시작 시 충전 상태를 종료한다.
- Main 버전은 `Vm1.0.11.0`이다.
- 승인된 Release clean build 성공: application 891,105 bytes, 동적 메모리 153,848 bytes, 출력 `bin/release/x64/Vm1.0.11.0/DF_Main.ino.bin` 891,472 bytes.
- 프로토콜 테스트, 플래시 및 실제 충전·게임 장비 검증은 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 11-S 충전 보정 범위 수정·TM V046 완료 (2026-09-04)

- 충전 거치 자세의 Roll/Pitch를 게임 영점으로 빼던 처리를 제거했다. 따라서 거치대에서 보정한 뒤 장비를 정상 자세로 들었을 때 거치 자세 차이로 게임 좌우값이 ±45도에 고정되지 않는다.
- 충전 기준 보정은 정지 상태의 자이로 X/Y/Z 바이어스만 수집한다. 게임 좌우값은 현재 Roll과 Z축 자이로 기반 가상 Roll을 합산한다.
- 보정 완료 표시는 거치 자세 R/P 대신 `GX/GY/GZ` 바이어스를 보여준다.
- TM 소스 버전을 `V046`으로 변경했다. 빌드·게시·self-test·실제 장비 검증은 사용자 사전 승인을 받지 않아 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 11-R TM 충전 기준 보정·Yaw 가상 Roll·V045 완료 (2026-09-04)

- TM 릴/IMU 화면에 기존 3D용 `0점 보정`과 별도의 `충전 기준 보정` 버튼을 추가했다. 이번 TM 단계에서는 작업자가 충전·정지 상태를 확인하고 버튼으로 시작한다.
- 버튼 실행 후 자이로 크기 3deg/s 이하의 정지 상태를 3초 확인하고 2초 동안 Roll/Pitch 기준과 자이로 바이어스를 평균한다. 수집 중 움직이면 정지 확인부터 다시 시작한다.
- `$09` 뒤 3개 자이로 값 중 Z축을 이용해 dead zone 2deg/s, 감도 0.65, 최대 ±30도, 정지 시 0.9초 감쇠 조건의 가상 Roll을 만들고 보정 Roll과 합쳐 ±45도의 게임 좌우값을 표시한다. Yaw 좌회전 양수는 기존 Roll 좌회전 음수와 맞도록 부호를 반전한다.
- 보정 상태, 가상 Roll, 게임 좌우값을 TM에 표시한다. IMU 출력 ON에서만 버튼이 활성화되며 연결 해제 시 TM 보정 상태를 초기화한다.
- TM 소스 버전을 `V045`로 변경했다. 빌드·게시·self-test·장비 검증은 사용자 사전 승인을 받지 않아 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 11-Q IMU Pitch 회전 방향·TM V044 완료 (2026-09-04)

- IMU 시각화의 Pitch 회전 부호를 반전해 양의 Pitch에서 기존의 앞으로 숙이는 방향이 아니라 반대 방향으로 꺾이도록 수정했다.
- Roll과 Yaw 회전 방향 및 0점 보정 계산은 유지했다.
- TM 소스 버전을 `V044`로 변경했다. 빌드·게시·self-test·실제 장비 방향 검증은 사용자 사전 승인을 받지 않아 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 11-P IMU FRONT 반전·3축 화살표·TM V043 완료 (2026-09-04)

- FRONT 기준을 화면 바깥쪽 `+Z` 면이 아니라 화면 안쪽을 향하는 모델 `-Z` 축으로 변경했다.
- 면 위 글자를 제거하고 박스 중심에서 FRONT 적색, TOP 파란색, RIGHT 녹색 굵은 화살표가 뻗도록 표시했다. 각 화살표는 보정된 박스 회전과 함께 움직인다.
- TM 소스 버전을 `V043`으로 변경했다. 빌드·게시·self-test·실제 화면 및 장비 검증은 사용자 사전 승인을 받지 않아 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 11-O IMU 보정 회전·엔코더 화면 배치·TM V042 완료 (2026-09-04)

- 0점 보정 후 화면에 표시되는 상대 Roll/Pitch/Yaw 값을 박스 회전에도 직접 적용해 보정값과 시각 자세가 일치하도록 수정했다.
- IMU 회전 화면과 컨테이너 높이를 각각 150px/155px로 줄여 아래의 엔코더 회전 제목과 회전 카운트 행이 화면 안에 들어오도록 조정했다.
- TM 소스 버전을 `V042`로 변경했다. 빌드·게시·self-test·실제 화면 및 장비 검증은 사용자 사전 승인을 받지 않아 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 11-N Visual Studio 직접 게시 구성 완료 (2026-09-04)

- `testModule/Properties/PublishProfiles/FolderProfile.pubxml`을 추가하고 프로젝트 기본 게시 프로필로 연결했다.
- Visual Studio의 `DFTestModule` 프로젝트에서 `게시`를 실행하면 Release/x64/win-x64, self-contained, 단일 EXE 조건으로 `C:\Projects\VS\DF_FW_Code\CodexManage\bin\testmodule\Release\win-x64\DFTestModule_V041.exe`를 만들도록 `PublishUrl`과 `PublishDir`을 고정했다.
- 실제 게시·빌드·실행 검증은 사용자 사전 승인을 받지 않아 수행하지 않았다. 활성 구현 잔여 작업은 0개다.

## 작업 11-M IMU 기준 자세·0점 보정·TM V041 완료 (2026-09-04)

- 책상 위 기준 자세에서 `FRONT`가 앞, `TOP`이 위, `RIGHT`가 오른쪽으로 보이도록 고정 원근 카메라와 입체 투영을 적용했다.
- Roll/Pitch/Yaw를 회전 행렬로 합성하고, `0점 보정` 시 현재 자세의 역행렬을 이후 자세에 적용해 실물의 상대 회전처럼 표시한다.
- `0점 보정` 버튼은 IMU 출력 ON 명령 송신 성공 후에만 활성화되며, 자세 데이터가 아직 없으면 실행하지 않고 안내한다. 연결 해제 시 보정값과 활성 상태를 초기화한다.
- TM 소스 버전을 `V041`로 변경했다. 빌드·self-test·실제 장비 방향 확인은 사용자 사전 승인 범위에 포함되지 않아 수행하지 않았으며, 마지막 생성 실행 파일은 기존 `DFTestModule_V040.exe`다.
- 활성 구현 잔여 작업은 0개다.

## 작업 11-L IMU 자세 표시·TM V040 완료 (2026-09-04)

- 기존 `$09` 수신의 Roll/Pitch/Yaw를 이용해 IMU 데이터 출력 ON 상태에서 함께 회전하는 박스를 릴/IMU 화면 우측에 추가했다. OFF에서는 마지막 자세를 유지한다.
- 기존 IMU 명령과 데이터 형식은 변경하지 않았다.
- TM 버전을 `V040`으로 설정하고 창 제목과 게시 파일명을 `DFTestModule_V040.exe`로 연결했다.
- 사용자 승인 후 Release 단일 EXE 게시가 성공해 `bin/testmodule/Release/win-x64/DFTestModule_V040.exe`를 생성했다. self-test와 실제 장비 회전 방향 검증은 미실행이다.
- 활성 구현 잔여 작업은 0개다.
